...
class SocketHandler(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                break
            debug("Received cmd: " + str(cmd) + " len: " + str(len(cmd)))
            if len(cmd) == 0:
                break
    ...
    def executeCommand(self, cmd):
        debug("Calling executeCommand() with  cmd: " + str(cmd)
        if cmd[:-1] == "go":  # remove trailing "\0"
            if GPIO.input(P_BUTTON) == GPIO.LOW:
                state = "Button pressed"
            else:
                state = "Button released"
            print("Reporting current state:" + str(state))
...
def TCP(car):
    ...
    while True:
        ...
        while isConnected:
            ...
            time.sleep(.025)
        car.isConnected, isConnected = False
        print("disconnected")
